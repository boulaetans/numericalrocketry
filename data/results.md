...
`nose_shell_mass_cg_m()` and `fin_set_mass_cg_m()` for that investigation). The exact frustum-slice and shoelace-formula integrals behind those two functions, worked out symbol by symbol, are in Section 2.3 of the [project report](../docs/numericalrocketry_report.pdf); the inertia-tensor formulas (solid cylinder vs. hollow tube, parallel-axis correction) that turn this same mass table into the rotational dynamics' inertia tensor are in Section 2.4.
