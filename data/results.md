...
| Burnout | t = 0.81 s | t = 0.81 s | t ≈ 0.57 s *(est.)* | exact | 42.53% |
| Apogee altitude | 101.25 m | 101.29 m | 111.2 m | -0.04 m (0.04%) | -9.95 m (8.95%) |
| Apogee time | 4.7663 s | 4.766 s | 4.76 s | ~1 ms | ~6 ms (0.13%) |
| Max velocity | 45.53 m/s | 45.53 m/s | 35.4 m/s *(see README note)* | ~0.005 m/s (0.01%) | +10.13 m/s (28.61%) |
| Recovery deploy | 5.8163 s | 5.811 s | not detectable in this log | ~5 ms | - |
| Touchdown | 27.116 s | 27.194 s | t ≈ 41.06 s *(est.)* | -0.08 s (0.29%) | -13.94 s (33.96%) |
| Liftoff mass | 166.041 g | 166 g | not measured | +0.04 g (0.03%) | - |
| CG location | 35.52 cm | 35.7 cm | not measured | -0.18 cm (0.50%) | - |

The **NR vs. Real gap** column is exactly the "real flight vs. simulation" comparison discussed in the README's Results section - apogee altitude's 8.95% gap is the number behind that whole discussion of dynamic-pressure/altimeter error, not a simulator inaccuracy (both NumericalRocketry and OpenRocket agree with each other to 0.04%, see above). Ignition, rail clear, recovery deploy, liftoff mass, and CG have no real-flight measurement to compare against at all, so there's no percentage to compute for those rows.

Burnout (42.53%) and touchdown (33.96%) look like much bigger misses than they actually are: both real-flight values are `(est.)`, derived from a different definition than what NumericalRocketry/OpenRocket report (burnout as "thrust drops below drag+weight" vs. total propellant consumption; touchdown as "speed settles near zero" vs. the flight computer's own, much-delayed "landed" flag). They're measuring related but not identical events, so a large gap there reflects that definitional difference more than actual trajectory error - unlike apogee and max velocity, which are directly comparable across all three.
