...
**Interactive version.** **[Open the interactive version](https://htmlpreview.github.io/?https://github.com/boulaetans/numericalrocketry/blob/main/legacy/comparison_interactive.html)** of [`comparison_interactive.html`](comparison_interactive.html), with the same data but draggable zoom/pan, hover tooltips showing exact values, and click-to-toggle lines in the legend. Works fully offline too: download the file and open it locally.

The exact update formula for each of the three integrators (forward Euler, the two-step Adams-Bashforth predictor with trapezoidal Adams-Moulton corrector, and classical RK4), along with the 1D model's own simplified drag-coefficient assembly, is written out equation by equation in Section 4 of the [project report](../docs/numericalrocketry_report.pdf).
